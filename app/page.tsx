...
/**
 * Step — single-page MVP UI v2.
 *
 * Improvements over v1:
 * - Skills + interests are clickable chips (pre-built common options +
 *   custom input) instead of comma-separated text.
 * - Selects have explicit light bg / dark text for legibility in any
 *   color scheme.
 * - Error box is always visible at top of form when present.
 * - Submit button has a clear disabled state.
 */
...
// Common skills users can pick. Tweak as we learn what real users have.
...
export default function HomePage() {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      if (!res.ok) {
        ...
        throw new Error(
          errBody.message ?? errBody.error ?? `API ${res.status}`,
        );
      }
  ...
  return (
    <main className="mx-auto flex min-h-screen max-w-3xl flex-col px-6 py-10 md:py-14">
      ...
      {phase === "error" && (
        <ErrorView message={errorMsg ?? "Something went wrong."} onReset={handleReset} />
      )}
  ...
  return (
    <section className="flex flex-col gap-8">
      <div>
        ...
        <p className="mt-3 max-w-xl text-lg text-ink-200/90 dark:text-ink-200/70">
...
function ChipPicker(p: ChipPickerProps) {
  // Combine suggestions + any user-added customs, but keep them visible in
  // a single list so the user sees what they've selected.
